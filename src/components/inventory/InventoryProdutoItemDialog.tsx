...
import type { ItemEstoqueProduto, ItemEstoqueProdutoCreate, ItemEstoqueProdutoUpdate } from "@/app/features/product-items/types";

type Props = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void | Promise<void>;
  idEstoqueProduto: string;
  editingItem?: ItemEstoqueProduto | null;
};
...
export const InventoryProdutoItemDialog = ({ open, onOpenChange, onSuccess, idEstoqueProduto, editingItem }: Props) => {
  const [lote, setLote] = useState("");
  const [dataProducao, setDataProducao] = useState("");
  const [dataValidade, setDataValidade] = useState("");
  const [quantidade, setQuantidade] = useState("0");
  const [quantidadeReservada, setQuantidadeReservada] = useState("0");
  ...
  useEffect(() => {
    if (open) {
      if (editingItem) {
        ...
        setQuantidadeReservada(String(editingItem.quantidade_reservada));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      if (editingItem) {
        const data: ItemEstoqueProdutoUpdate = {
          lote: lote.trim(),
          ...
          quantidade_reservada: parseFloat(quantidadeReservada) || 0,
          quantidade_minima: parseFloat(quantidadeMinima) || 0,
          custo_unitario: parseFloat(custoUnitario),
          localizacao: localizacao.trim() || null,
          observacoes: observacoes.trim() || null,
        };
        await productItemsApi.update(editingItem.id, data);
      } else {
        const data: ItemEstoqueProdutoCreate = {
          id_estoque_produto: idEstoqueProduto,
          lote: lote.trim(),
        ...
        await productItemsApi.create(data);
      }
      await onSuccess();
      onOpenChange(false);
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md gap-6">
        <DialogHeader>
          <DialogTitle>{editingItem ? "Editar lote de produto" : "Novo lote de produto"}</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <FieldGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field>
              <FieldLabel>Quantidade reservada</FieldLabel>
              <Input type="number" step="0.01" min="0" value={quantidadeReservada} onChange={(e) => setQuantidadeReservada(e.target.value)} />
            </Field>
            <Field>
            ...
            <div className="sm:col-span-2">
              <Field>
                ...
                <Input value={localizacao} onChange={(e) => setLocalizacao(e.target.value)} placeholder="Ex: EST-FIN-A1" />
              </Field>
            </div>
            <div className="sm:col-span-2">
              <Field>
